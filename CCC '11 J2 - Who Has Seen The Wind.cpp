...
int main(){
	...
	int h, m; cin >> h >> m; bool b = true;
	for(int t = 1; t <= m; t++){
        if(-6*t*t*t*t+h*t*t*t+2*t*2+t <= 0){cout << "The balloon first touches ground at hour:\n" << t; b = false; break;}
	}
	if(b) cout << "The balloon does not touch ground in the given time.";
