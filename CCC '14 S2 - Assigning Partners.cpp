...
int main(){
	...
	int n; cin >> n; string l1[n], l2[n];
	for(int i = 0; i < n; i++){
        string temp; cin >> temp; l1[i] = temp;
	}
	for(int i = 0; i < n; i++){
        string temp; cin >> temp; l2[i] = temp;
	}
	string ans = "good";
	map<string, string> mp; set<pair<string, string>> st;
	for(int i = 0; i < n; i++){
        mp.insert({min(l1[i], l2[i]), max(l1[i], l2[i])}); mp.insert({max(l1[i], l2[i]), min(l1[i], l2[i])});
        st.insert({min(l1[i], l2[i]), max(l1[i], l2[i])}); st.insert({max(l1[i], l2[i]), min(l1[i], l2[i])});
        if(l1[i] == l2[i]) ans = "bad";
    }
	if(mp.size() == st.size()) cout << ans;
	else cout << "bad";
